...
def backtracking(index, current, status):
    ...
    if index < n - 1:
        if status == 0:
            ...
            backtracking(index+2, current + operations[index] + operations[index+1], 0)
        else:
            backtracking(index+2,current + operations[index] + ")" +  operations[index+1], 0)
            # backtracking(index+2, current + operations[index] + operations[index+1], 1) #  괄호 안에는 연산자 한 개만 가능하기 떄문
